...
namespace PalettePal_web
{
    ...
    {
        // KMeans algorithm C# realization by James McCaffrey
        // https://visualstudiomagazine.com/articles/2013/12/01/k-means-data-clustering-using-c.aspx
        public static int[] KMeans(double[][] data, int numClusters)
        {
            // k-means clustering
            ...
            //    update means
            // end loop
            int[] clustering = Array.Empty<int>();
            for (int seed = 0, ct = 0; ct < 2; seed++)
            {
                clustering = InitClustering(data.Length, numClusters, seed); // semi-random initialization
        ...
        public static double[][] FindCentroids(double[][] rawData, int[] labels, int numClusters, int[] hist)
        {
            ...
            {
                centroids[cluster] = new double[3];
            }
            for (int i = 0; i < rawData.Length; i++)
            {
                for (int j = 0; j < 3; j++)
                {
                    centroids[labels[i]][j] += rawData[i][j];
                }
            }
            for (int cluster = 0; cluster < numClusters; cluster++)
            {
                for (int j = 0; j < 3; j++)
                {
                    centroids[cluster][j] /= hist[cluster];
                }
            }
            return centroids;
        }
        ...
        {
            return labels.GroupBy(x => x).OrderBy(x => x.Key).Select(x => x.Count()).ToArray();
        }

        // Modified Silhouette method with calculating distances to centroids instead of other points. Produces nearly the same results
        public static double CalculateSilhouetteScore(double[][] rawData, int[] labels, int numClusters)
        {
            double[][] centroids = FindCentroids(rawData, labels, numClusters, Histogram(labels));
            double[,] distances = new double[rawData.Length, numClusters];
            for (int i = 0; i < rawData.Length; i++)
            {
                for (int cluster = 0; cluster < numClusters; cluster++)
                {
                    distances[i, cluster] += Distance(rawData[i], centroids[cluster]);
                }
            }

            double[] ownDist = new double[rawData.Length]; // same cluster average distances
            double[] diffDist = new double[rawData.Length]; // min different cluster distances
            Array.Fill(diffDist, 1000000.0);
            for (int i = 0; i < rawData.Length; i++)
            {
                for (int cluster = 0; cluster < numClusters; cluster++)
                {
                    if (cluster != labels[i])
                    {
                        if(distances[i, cluster] < diffDist[i]) diffDist[i] = distances[i, cluster];
                    }
                    else
                    {
                        ownDist[i] = distances[i, cluster];
                    }
                }
            }
            ...
            for (int i = 0; i < rawData.Length; i++)
            {
                total += (diffDist[i] - ownDist[i]) / Math.Max(ownDist[i], diffDist[i]);
            }
